...
myLayout = defaultMultiWindow ||| noBorders Full
...
myXDefaultPrompter :: String -> String
myXDefaultPrompter identity = identity

myXDefaultSorter :: String -> [String] -> [String]
myXDefaultSorter identity identityList = identityList
...
myXPromptConfig =
  XPC
    ...
    , historyFilter = id
    , defaultText = []
    , searchPredicate = myXPromptSearchPredicate
    , defaultPrompter = myXDefaultPrompter
    , sorter = myXDefaultSorter
    }
...
instance XPrompt MyPrompt where
  ...
  modeAction (MyPrompt _ w) completion _ = case completion of
    ...
    "netflix" -> spawn "google-chrome https://netflix.com"
...
defaults = def
  ...
  , handleEventHook = myHandleEventHook --handleEventHook def <+> docksEventHook
